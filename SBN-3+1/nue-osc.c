...
int main(int argc, char *argv[])
{
  ...
  /* char* MYFILE=""; */ 
  char* MYFILE="./results/ev-osc-SBN-nue.dat"; /* if empty, write to screen, otherwise to file name given here */
  FILE* stream;
  if(strlen(MYFILE)>0) stream=fopen(MYFILE, "w+");
   else stream = stdout;
 
  char* MYFILE1="./results/chi-osc-SBN-nue.dat"; /* if empty, write to screen, otherwise to file name given here */
  FILE* gabi;
 ...
 glbInitExperiment("./detectors/ICARUS/SBN_icarus_nue.glb",&glb_experiment_list[0],&glb_num_of_exps); 
 glbInitExperiment("./detectors/MicroBooNE/SBN_mboone_nue.glb",&glb_experiment_list[0],&glb_num_of_exps);
 glbInitExperiment("./detectors/SBND/SBN_lar_nue.glb",&glb_experiment_list[0],&glb_num_of_exps); 
...
/* double ener=0;
  double i;
  for(i=0.2;i<3.1;i=i+0.01){
  ener=i;
  double resul=glbVacuumProbability(2,1,+1,ener,0.540);
  fprintf(stream,"%f %f \n",ener,resul);
...
// for(w=0;w<n_bins;w++) fprintf(stream,"%g %g %g %g \n", center_bin_N[w], size_bin_N[w] ,chan1_rates_N[w] +chan2_rates_N[w], bg_rates_N[w]);
 ...
 //  int rule_rate = glbShowRuleRates(stream, 2, 0, GLB_ALL, GLB_W_EFF, GLB_W_BG, GLB_W_COEFF, GLB_SIG); 
