...
#include "syst_func_mb_comb_osc.h"
...
int main(int argc, char *argv[])
{
 ...
 //readcov2();
  ...
  /* char* MYFILE=""; */ 
  char* MYFILE="./results/ev-osc-mb-comb2.dat"; /* if empty, write to screen, otherwise to file name given here */
  FILE* stream;
  if(strlen(MYFILE)>0) stream=fopen(MYFILE, "w+");
   else stream = stdout;
 
  char* MYFILE1="./results/chi-osc-mb-comb.dat"; /* if empty, write to screen, otherwise to file name given here */
  FILE* gabi;
  ...
  double true_thetame = asin(sqrt(0.918))/2;
  double true_stdm = 0.041;
  ...
  glbInitExperiment("MB_comb_osc.glb",&glb_experiment_list[0],&glb_num_of_exps);  
...
/*  fprintf(stream,"Simulated rates, experiment 0, rule 0: \n"); */
    ...
    double *true_rates_N = glbGetSignalRatePtr(0,1);
    double *bg_rates_N = glbGetBGRatePtr(0,1);
