...
int main(int argc, char *argv[])
{
 ...
 //readcov();
  ...
  /* char* MYFILE=""; */ 
  char* MYFILE="./results/ev-osc-MB-numu.dat"; /* if empty, write to screen, otherwise to file name given here */
  FILE* stream;
  if(strlen(MYFILE)>0) stream=fopen(MYFILE, "w+");
   else stream = stdout;
 
  char* MYFILE1="./results/chi-osc-MB-numu.dat"; /* if empty, write to screen, otherwise to file name given here */
  FILE* gabi;
  ...
  glbDefineChiFunction(&chiMB_nue,     0,        "chiMB_nue",     NULL);
 ...
 glbInitExperiment("MB_numu.glb",&glb_experiment_list[0],&glb_num_of_exps);  
...
/*  fprintf(stream,"Simulated rates, experiment 0, rule 0: \n"); */
    ...
    double *true_rates_1 = glbGetSignalRatePtr(0,0);
   ...
   for(w=0;w<n_bins;w++) fprintf(stream,"%g %g %g %g \n", center_bin_N[w], size_bin_N[w] ,true_rates_1[w], bg_rates_N[w]);
...
/* double z,y,chi2; 
   ...
   for(z=-4.0;z < 0.0 - 0.001; z=z+4.0/100)
   for(y=-2.0;y< 2.0+0.01;y=y+4.0/100) 
  {
