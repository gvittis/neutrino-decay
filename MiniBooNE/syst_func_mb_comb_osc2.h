	...
	for(int i=0; i< my_matrix->N1; i++)
	{
		for(int j=0; j< my_matrix->N1; j++)
		{
			if(i==j && i < 11){
                        ...
                        if(i==j && 30<= i && i < 44){
                           _Matrix_2[i][j]=_Matrix_1[i][j]*vec_P_a[i]*vec_P_a[j]+signal_fit_rates_a[i-30];}
			else{
