...
// readcov2();
  ...
  glbInitExperiment("MB_anue_decay.glb",&glb_experiment_list[0],&glb_num_of_exps);
  glbInitExperiment("MB_nue_decay.glb",&glb_experiment_list[0],&glb_num_of_exps); 
...
// Cria a variável que me auxiliará a levar os dados do mini_boone para a sistematica.
	...
	glb_experiment_list[0]->sys_on[0]->user_data=(void *) &matrizes;  
        ...
        int exp[2]={0,1};
        for(int i=0; i<2; i++){
        glb_experiment_list[i]->probability_user_data=(void *) &exp[i];} 
  ...
  glbSetRates();
   ...
   char* MYFILE="./results/ev-decay-mb-comb-d2.dat";                       
   FILE* stream;
   if(strlen(MYFILE)>0) stream=fopen(MYFILE, "w+");
   else stream = stdout;

    
   char* MYFILE2="./results/chi-decay-mb-comb-d2.dat"; 
   FILE* gabi;
  ...
  /* Compute chi^2 without correlations */
   ...
   for(a=-4; a <= 0;a=a+4.0/60)
    for(b=-2; b<= 2;b=b+4.0/60){  
