  ...
  int loadplus1(void)
  {
   ...
   while (q < en) 
   {
    for(qi=0; qi<flu;qi++) // counter of columns of the file ntorres gives the number of columns
     { 
      state=fscanf(file,"%lf", &icaplus[q][qi]); // scan the column functions
   //  printf("%g \n",miniplus[q][qi]);
     }
    q++;
   }
...
double icap_flux(double E, int f /* 1 a 6 */)
{
  int col = f;
  int n_steps = 501;
  double result;
  ...
  int k=0;
  while (k <= n_steps  &&  icaplus[k][0] < E)
    k++;
  if (k <= 0 || k > n_steps)
    return 0.0;
  else
  {
    double E_lo    = icaplus[k-1][0];
    double E_up    = icaplus[k][0];
    double flux_lo = icaplus[k-1][col];
    double flux_up = icaplus[k][col];
...
double complex decay_icap_1(double x, double y, int f, double Erec, double L){
     double complex decay=0;
     double En;
     double binwidth = 0.009888;
     double complex dPdE1=0;
             
for (En = Erec + binwidth; En <  icaplus[500][0]; En= En+binwidth)
{
  dPdE1 = 0.5*2*Erec/(En*En)*x*(1-exp(-5.0677*square(y)*L/(/*32*/16*PI*En)));
  
  decay = decay + dPdE1*icap_flux(En,f)*binwidth;
} 
     return decay;
}

double complex decay_icap_2(double x, double y, int f, double Erec, double L){
     double complex decay=0;
     double En;
     double binwidth = 0.009888;
...
for (En = Erec + binwidth; En < icaplus[500][0]; En= En+binwidth)
{
  ...
  decay = decay + dPdE2*icap_flux(En,f)*binwidth;
 }
     return decay;
}
                          ...
                          double filter_sigma, void *user_data)
{
 ...
 /*nu_e*/    P[1][0] = decay_larp_1(u,mg,2,E,L) /*nu_mu*/ + decay_larp_2(u,mg,5,E,L) /*anu_mu*/;
     
 /*nu_mu*/   P[1][1] = (square(1-u)+square(u)*exp(-5.0677*square(mg)*L/(16*PI*E)))*larp_flux(E,2);
}
     
if(fabs(L-0.47)<1.0e-8){ 
 /*nu_e*/    P[1][0] = decay_microp_1(u,mg,2,E,L) /*nu_mu*/ + decay_microp_2(u,mg,5,E,L) /*anu_mu*/;
     
 /*nu_mu*/   P[1][1] = (square(1-u)+square(u)*exp(-5.0677*square(mg)*L/(16*PI*E)))*microp_flux(E,2);
}
if(fabs(L-0.6)<1.0e-8){
 /*nu_e*/    P[1][0] = decay_icap_1(u,mg,2,E,L) /*nu_mu*/ + decay_icap_2(u,mg,5,E,L) /*anu_mu*/;
     
 /*nu_mu*/   P[1][1] = (square(1-u)+square(u)*exp(-5.0677*square(mg)*L/(16*PI*E)))*icap_flux(E,2);
}
}
    ...
    {
    if(fabs(L-0.11)<1.0e-8){
 /*anu_e*/    P[1][0] = decay_larp_1(u,mg,5,E,L) /*anu_mu*/ + decay_larp_2(u,mg,2,E,L) /*nu_mu*/;
     
 /*anu_mu*/   P[1][1] = (square(1-u)+square(u)*exp(-5.0677*square(mg)*L/(16*PI*E)))*larp_flux(E,5);
}

if(fabs(L-0.47)<1.0e-8){ 
/*anu_e*/    P[1][0] = decay_microp_1(u,mg,5,E,L) /*anu_mu*/ + decay_microp_2(u,mg,2,E,L) /*nu_mu*/;
     
 /*anu_mu*/   P[1][1] = (square(1-u)+square(u)*exp(-5.0677*square(mg)*L/(16*PI*E)))*microp_flux(E,5);
}

if(fabs(L-0.6)<1.0e-8){
 /*anu_e*/    P[1][0] = decay_icap_1(u,mg,5,E,L) /*anu_mu*/ + decay_icap_2(u,mg,2,E,L) /*nu_mu*/;
     
 /*anu_mu*/   P[1][1] = (square(1-u)+square(u)*exp(-5.0677*square(mg)*L/(16*PI*E)))*icap_flux(E,5);
}
    }
