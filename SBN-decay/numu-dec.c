...
{ 
 ...
 loadplus3();
  ...
  glbInitExperiment("./detectors/ICARUS/SBN_icarus_m.glb",&glb_experiment_list[0],&glb_num_of_exps);
  glbInitExperiment("./detectors/MicroBooNE/SBN_mboone_m.glb",&glb_experiment_list[0],&glb_num_of_exps);
  glbInitExperiment("./detectors/SBND/SBN_lar_m.glb",&glb_experiment_list[0],&glb_num_of_exps); 
...
// Cria a variável que me auxiliará a levar os dados do mini_boone para a sistematica.
  ...
  glbSetRates();
   ...
   char* MYFILE="./results/ev-decay-sbn-numu.dat"; 
   FILE* stream;
   ...
   char* MYFILE2="./results/chi-decay-sbn-numu.dat"; 
   FILE* gabi;
  ...
  /* Compute chi^2 without correlations */
   ...
   for(k=-4;k < 0.1;k=k+4.0/50)
    for(l=-2;l < 2.1;l=l+4.0/50){  
